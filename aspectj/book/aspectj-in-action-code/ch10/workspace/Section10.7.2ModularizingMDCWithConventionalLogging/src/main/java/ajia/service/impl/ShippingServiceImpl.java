...
public class ShippingServiceImpl implements ShippingService {
    ...
    public void processOrder(Order order) {
        logger.log(Level.INFO, "Processing order " + order.getId());
        
        Package packageToSend = createPackage(order);
        logger.log(Level.INFO, "Notifying courier " + "service with " + packageToSend);
        Tracking tracking = courierService.send(packageToSend);
        logger.log(Level.INFO, "Sending email with " + tracking);
